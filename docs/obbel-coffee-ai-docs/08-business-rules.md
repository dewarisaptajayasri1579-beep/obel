...
## BR-037 — Penomoran nomor bukti
Semua nomor bukti (invoice/receipt/dokumen) di seluruh modul WAJIB memakai format sekuensial sederhana: `PREFIX-000001`, naik satu per dokumen, diambil dari nomor TERBESAR yang sudah ada per prefix (bukan dari jumlah baris), lalu di-pad 6 digit. Ini pola yang sudah dipakai Tambah Stok Gudang (`TRM-000001`) dan menjadi acuan untuk semua jenis dokumen lain — Penjualan (`OBL`), Serah Terima/Terima Stok (`DIST`), Retur (`RTN`), Restock Request (`RSTK`), Stock Opname (`OPN`), Reconciliation Case (`RECON`), Refund (`RFD`), Stock Movement (`MOV`/`ADJ`), dst.

Dilarang memakai skema timestamp+random (mis. `DIST-LX2K3A-1F2B3C`) atau skema lain yang tidak berurutan dan tidak mudah dibaca manusia — nomor bukti harus enak dilihat dan diucapkan di lapangan (Admin/Petugas Booth), bukan cuma untuk keperluan uniqueness teknis. Implementasi acuan: `backend/src/common/doc-no.ts` (`nomorSekuensialBerikutnya`, `alokasikanNomorSekuensial`, `nomorMovementBerikutnya`) — setiap service query nomor tertinggi miliknya sendiri dalam transaksi yang sama lalu increment satu, dengan retry-on-unique-constraint-violation (pola `StockReceiptsService.nomorBerikutnya`) untuk menangani dua request bersamaan yang kebetulan membaca nomor tertinggi yang sama. Dokumen revisi/versi baru (mis. hasil `reviseDistribution`, `reviseSale`) juga WAJIB mengambil nomor baru lewat generator ini, bukan format lama — sebuah dokumen versi baru bukan alasan untuk memakai skema penomoran berbeda dari dokumen aslinya.
