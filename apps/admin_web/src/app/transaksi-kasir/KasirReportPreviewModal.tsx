...
import { api, type FilterLaporanKasir } from "@/lib/api-client";

/// Pratinjau PDF daftar Transaksi Booth - Kasir — pola sama persis dengan
/// SerahTerimaReportPreviewModal.tsx (Serah Terima Stok).
export function KasirReportPreviewModal({
  isOpen,
  onClose,
  filter,
}: {
  ...
  }, [isOpen, filter.q, filter.status, filter.boothName, filter.staffName, filter.periodeAwal]);

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Pratinjau Daftar Transaksi Booth - Kasir" size="lg">
      <div className="space-y-3">
        ...
        <div className="h-[60vh] rounded-xl border border-slate-200 dark:border-line overflow-hidden bg-slate-50 dark:bg-surface-hover">
          {gagal ? (
            ...
            <iframe src={url} title="Pratinjau PDF daftar Transaksi Booth - Kasir" className="w-full h-full" />
          ) : (
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          <button
            ...
            disabled={!url}
            onClick={() => {
              const frame = document.querySelector<HTMLIFrameElement>('iframe[title="Pratinjau PDF daftar Transaksi Booth - Kasir"]');
