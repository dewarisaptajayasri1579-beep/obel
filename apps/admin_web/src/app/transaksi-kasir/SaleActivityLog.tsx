...
const AKSI_LABEL: Record<string, string> = {
  CREATE: "Draft disimpan",
  PAID: "Dibayar",
  VOID: "Dibatalkan",
  POST_REVISION: "Revisi diposting",
  PAYMENT_METHOD_CHANGED: "Metode bayar diubah",
  REFUND: "Refund",
};

const AKSI_WARNA: Record<string, string> = {
  CREATE: "bg-slate-100 dark:bg-surface-hover text-slate-600 dark:text-fg-muted",
  PAID: "bg-brand-50 dark:bg-brand-500/10 text-[var(--brand-700)] dark:text-brand-400",
  VOID: "bg-rose-50 dark:bg-rose-900/20 text-rose-700 dark:text-rose-400",
  POST_REVISION: "bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400",
  PAYMENT_METHOD_CHANGED: "bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400",
  REFUND: "bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400",
};
...
/// Panel "Riwayat Aktivitas" transaksi kasir — pola sama persis dengan
/// PenerimaanActivityLog.tsx (Tambah Stok Gudang), sumbernya
/// ActivityLogService lewat GET /sales/:id/activity-log.
export function SaleActivityLog({ saleId }: { saleId: string }) {
