...
import { api, type FilterLaporanSerahTerima } from "@/lib/api-client";

/// Pratinjau PDF daftar Serah Terima Stok — pola sama persis dengan
/// PenerimaanReportPreviewModal.tsx (Tambah Stok Gudang).
export function SerahTerimaReportPreviewModal({
  isOpen,
  onClose,
  filter,
}: {
  isOpen: boolean;
  onClose: () => void;
  filter: FilterLaporanSerahTerima;
}) {
  ...
  useEffect(() => {
    ...
    api
      .getStockHandoverReport("pdf", filter)
      .then((blob) => {
        if (batal) return;
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Pratinjau Daftar Serah Terima Stok" size="lg">
      <div className="space-y-3">
        ...
        <div className="h-[60vh] rounded-xl border border-slate-200 dark:border-line overflow-hidden bg-slate-50 dark:bg-surface-hover">
          {gagal ? (
            ...
            <iframe src={url} title="Pratinjau PDF daftar Serah Terima Stok" className="w-full h-full" />
          ) : (
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          <button
            ...
            disabled={!url}
            onClick={() => {
              const frame = document.querySelector<HTMLIFrameElement>('iframe[title="Pratinjau PDF daftar Serah Terima Stok"]');
