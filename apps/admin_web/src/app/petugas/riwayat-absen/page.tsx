  ...
  return (
    <div className="min-h-screen bg-[#F7F9F6]">
      <div className="bg-white px-5 pt-5 pb-4">
        ...
        <p className="text-sm text-slate-500 mt-1">Lihat riwayat kehadiran dan aktivitas kerja Anda.</p>
      </div>

      <div className="p-4">
        <div className="rounded-2xl bg-white border border-slate-200 p-4 flex items-center justify-between mb-4">
          <div>
            <p className="text-sm text-slate-500">Total Hadir Bulan Ini</p>
            <p className="font-extrabold text-2xl mt-1" style={{ color: GREEN }}>
              {history.totalHadir} hari
            </p>
            <p className="text-sm text-slate-400 mt-0.5">Dari {history.totalHariKerja} hari berjalan</p>
          </div>
        ...
        <div className="flex flex-col gap-2.5">
          {history.items.length === 0 && (
            <p className="text-base text-slate-500 text-center py-10">Belum ada riwayat absen bulan ini.</p>
          )}
            ...
            return (
              <div key={item.id} className="rounded-xl bg-white border border-slate-200 p-3.5">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-bold rounded-full px-2.5 py-0.5" style={{ backgroundColor: status.bg, color: status.fg }}>
                    {status.label}
                  </span>
                  <span className="text-sm text-slate-400">{formatTanggalJakarta(item.businessDate)}</span>
                </div>
                <p className="font-semibold text-base text-slate-900">{item.boothName}</p>
