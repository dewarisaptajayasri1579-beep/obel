...
/** Controlled kalau `value` diberikan, uncontrolled (pakai state internal) kalau tidak. */
...
export const TabList: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = "" }) => (
  <div role="tablist" className={`flex items-center gap-1 border-b border-slate-200/80 dark:border-line overflow-x-auto ${className}`}>
    {children}
  </div>
);
