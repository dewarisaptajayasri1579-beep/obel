...
export const ColumnVisibilityMenu: React.FC<ColumnVisibilityMenuProps> = ({ columns, isVisible, onToggle }) => {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 px-3.5 h-9 rounded-xl bg-white/80 dark:bg-surface hover:bg-white dark:hover:bg-surface-hover border border-slate-200/90 dark:border-line shadow-sm text-xs font-bold text-slate-600 dark:text-fg-secondary transition-all cursor-pointer"
      >
            ...
            >
              <span
                className={`w-4 h-4 rounded-md border flex items-center justify-center flex-shrink-0 ${
                  isVisible(col.key) ? "bg-brand-600 border-brand-600" : "border-slate-300 dark:border-line-strong"
                }`}
              >
