...
export const ErrorState: React.FC<ErrorStateProps> = ({ icon: Icon, code, title, description, action }) => {
  return (
    <div className="min-h-screen w-full bg-app-mesh flex items-center justify-center p-6 relative overflow-hidden">
      <div className="dark:hidden fixed -top-40 -left-40 w-[500px] h-[500px] bg-brand-400/20 rounded-full blur-3xl pointer-events-none" />
      <div className="dark:hidden fixed -bottom-40 -right-40 w-[500px] h-[500px] bg-brand-500/20 rounded-full blur-3xl pointer-events-none" />

      <div className="relative z-10 w-full max-w-md glass-card rounded-[32px] p-8 sm:p-10 text-center flex flex-col items-center">
        <div className="w-16 h-16 rounded-full bg-[#F0FFF8] dark:bg-brand-500/10 text-[var(--brand-700)] dark:text-[var(--accent-primary)] border border-brand-100 dark:border-brand-500/30 flex items-center justify-center shadow-sm mb-5">
          <Icon className="w-8 h-8 stroke-[2]" />
        </div>
        {code && <p className="text-xs font-bold uppercase tracking-widest text-brand-600 dark:text-[var(--accent-primary)] mb-1.5">{code}</p>}
