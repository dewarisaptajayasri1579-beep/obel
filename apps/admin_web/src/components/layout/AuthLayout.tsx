...
export const AuthLayout: React.FC = () => {
  return (
    <div className="min-h-screen w-full bg-app-mesh flex flex-col justify-between p-4 sm:p-6 lg:p-8 font-sans relative overflow-x-hidden">
            ...
            <div className="grid grid-cols-3 gap-3.5 pt-2">
              {APP_CONFIG.authFeatures.map(({ icon: Icon, title, desc }) => (
                <div key={title} className="glass-mockup-feature-card p-3.5 sm:p-4 rounded-2xl flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-800 dark:text-fg">{title}</h4>
                    <p className="text-[11px] text-slate-600 dark:text-fg-muted font-medium leading-tight mt-0.5">{desc}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <footer className="mt-6 flex flex-col items-center gap-1 text-center text-xs text-slate-600 dark:text-fg-muted font-semibold z-10">
        <MotivationalQuote />
