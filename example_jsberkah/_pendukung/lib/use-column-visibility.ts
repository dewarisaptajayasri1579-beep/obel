...
export function useColumnVisibility(storageKey: string, columns: ColumnDef[]) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hasObsoleteKeys = saved.some((k) => !allKeys.includes(k));
      if (hasObsoleteKeys) {
        const merged = new Set([...validSaved, ...allKeys.filter((k) => !saved.includes(k))]);
        setVisible(merged);
        window.localStorage.setItem(`columns:${storageKey}`, JSON.stringify(Array.from(merged)));
      } else {
        setVisible(new Set(validSaved));
      }
    } catch {
  ...
  const toggle = (key: string) => {
    setVisible((prev) => {
      ...
      if (next.has(key)) {
        // Kolom terakhir tidak boleh ikut dimatikan — tabel tanpa satu kolom pun
        // cuma menyisakan baris kosong yang tidak bisa dipulihkan dari tabelnya
        // sendiri (menunya masih ada, tapi user sudah tidak punya petunjuk apa-apa).
