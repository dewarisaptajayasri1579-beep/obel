...
export interface SerahTerimaBarangPrintableLine {
  productCode: string;
  productName: string;
  qty: number;
}

export interface SerahTerimaBarangPrintableData {
  stbNumber: string | null;
  date: string;
  status: "MENUNGGU_KONFIRMASI" | "DIKONFIRMASI" | "DITOLAK" | null;
  salesName: string;
  warehouseName: string;
  notes: string;
  lines: SerahTerimaBarangPrintableLine[];
...
const STATUS_BADGE: Record<NonNullable<SerahTerimaBarangPrintableData["status"]>, { label: string; className: string }> = {
  MENUNGGU_KONFIRMASI: { label: "Menunggu Konfirmasi", className: "bg-amber-100 text-amber-800 border-amber-200" },
  DIKONFIRMASI: { label: "Dikonfirmasi", className: "bg-emerald-100 text-emerald-800 border-emerald-200" },
  DITOLAK: { label: "Ditolak", className: "bg-rose-100 text-rose-800 border-rose-200" },
};

const kolomRincian: KolomNotaItem<SerahTerimaBarangPrintableLine>[] = [
  { key: "no", header: "No.", align: "center", widthClass: "w-8", render: (_l, idx) => <span className="text-slate-500 font-medium">{idx + 1}</span> },
  { key: "kode", header: "Kode", render: (l) => <span className="font-mono font-semibold text-slate-700">{l.productCode}</span> },
  { key: "nama", header: "Nama Barang", render: (l) => <span className="font-semibold text-slate-800 truncate max-w-[260px] block">{l.productName}</span> },
  {
    key: "qty",
    header: "Qty",
    align: "right",
    widthClass: "w-20",
    render: (l) => <span className="font-bold text-slate-900 tabular-nums">{l.qty.toLocaleString("id-ID")}</span>,
  },
];

/** Dokumen STB gaya nota — kembaran `PurchaseOrderPrintable.tsx` (susunan, ukuran huruf,
 *  dekorasi disamakan persis, satu keluarga nota di app ini), disederhanakan: TIDAK ada
 *  kolom Harga/Subtotal/Total Rupiah (STB pemindahan tanggung jawab stok, bukan transaksi
 *  jual-beli) dan tanda tangan kanan "Diterima Sales" (bukan "Disetujui Supplier") karena STB
 *  diserahkan ke Sales, bukan dipesan ke Supplier.
 *
 *  Dipakai DUA tempat: preview sebelum simpan (`SerahTerimaBarangPreviewModal`, `stbNumber`/
 *  `status` null karena belum ada) dan salinan tersembunyi `.print-only` buat `window.print()`. */
export const SerahTerimaBarangPrintable: React.FC<{ data: SerahTerimaBarangPrintableData; className?: string }> = ({ data, className = "" }) => {
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex flex-row items-center justify-between gap-3 relative z-10">
        <div>
          <h1 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight leading-none">SERAH TERIMA BARANG</h1>
          <p className="text-[8.5px] font-semibold text-slate-400 tracking-[0.2em] uppercase mt-0.5">B A R A N G   D A R I   G U D A N G   K E   S A L E S</p>
          <p className="text-[9.5px] text-slate-500 mt-0.5 font-medium leading-tight">Stok baru pindah setelah Sales konfirmasi lewat aplikasi.</p>
        </div>
        ...
        <div className="rounded-lg border border-blue-100 bg-blue-50 py-1 px-2.5 min-w-[210px] text-[10.5px] shadow-2xs space-y-0.5">
          <div className="flex items-center justify-between">
            ...
            <span className={`font-extrabold font-mono tracking-wide ${data.stbNumber ? "text-slate-900" : "italic text-slate-400 font-normal"}`}>
              {data.stbNumber ?? "otomatis saat disimpan"}
            </span>
          </div>
          <div className="flex items-center justify-between">
        ...
        <div className="rounded-lg border border-blue-100 bg-blue-50 py-1.5 px-2.5">
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0">
              <p className="text-[8px] font-bold text-slate-500 uppercase tracking-wider">Diserahkan Ke Sales</p>
              <p className="text-xs font-extrabold text-slate-900 truncate leading-tight">{data.salesName || "—"}</p>
            </div>
          </div>
        </div>
        <div className="rounded-lg border border-slate-200/80 bg-slate-50 py-1.5 px-2.5">
          <p className="text-[8px] font-bold text-slate-500 uppercase tracking-wider">Dari Gudang</p>
          <p className="text-xs font-extrabold text-slate-900 truncate leading-tight mt-0.5">{data.warehouseName || "—"}</p>
        </div>
      </div>
      ...
      <NotaItemTable
        ...
        rows={data.lines}
        rowKey={(_l, idx) => idx}
        footer={{ label: `Jumlah ${data.lines.length} baris barang`, valueColumnKey: "qty", value: totalQty.toLocaleString("id-ID") }}
      />

      {/* TANDA TANGAN — 2 kolom: pembuatnya (Admin Gudang) dan Sales yang menerima. */}
      <div className="mt-2 grid grid-cols-2 gap-6 text-xs relative z-10">
        <div className="flex flex-col justify-between h-14">
          <p className="font-bold text-slate-800 text-[10px]">Diserahkan Oleh</p>
        ...
        <div className="flex flex-col justify-between h-14 border-l border-slate-200/80 pl-4">
          <p className="font-bold text-slate-800 text-[10px]">Diterima Sales</p>
          <div className="space-y-0.5 text-slate-600">
            <div className="border-b border-slate-300 w-full mb-0.5" />
            <p className="text-[9px] flex">
              <span className="inline-block w-10 flex-shrink-0">Nama</span>
              <span>: {data.salesName ? <span className="font-bold text-slate-900">{data.salesName}</span> : "...................................."}</span>
            </p>
            <p className="text-[9px] flex">
              <span className="inline-block w-10 flex-shrink-0">Tanggal</span>
              <span>: ....................................</span>
      ...
      <div className="mt-2 pt-1.5 flex flex-row items-end justify-between gap-3 relative z-10 border-t border-slate-100">
        <div className="rounded-lg bg-blue-50/70 border border-blue-100 py-1 px-2 max-w-sm flex items-start gap-1.5 text-slate-600">
          ...
          <div>
            ...
            <p className="text-[8.5px] leading-tight text-slate-600 mt-0.5">{data.notes || "Dokumen ini dicetak otomatis dari sistem sebagai bukti serah terima barang."}</p>
          </div>
        </div>
